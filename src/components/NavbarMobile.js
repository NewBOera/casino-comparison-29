...
export class NavbarMobile extends LitElement {
  ...
  static styles = css`
    ...
    .bg-menu-navbar {
      background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('https://cdn.best10ukspots.com/public/assets/navbar/background.webp');
        ...
        <nav class="flex z-50 relative justify-between h-[80px] bg-[#FFFFFF] sm:w-[640px] md:w-[768px] w-full">
          <div class="text-lg flex gap-4 items-center">
            <a class="flex items-center gap-2" href="${this.isLanding ? '#home-section' : 'index.html'}">
              <img class="object-contain w-[45px]" src="https://cdn.best10ukspots.com/public/assets/navbar/logo.webp" alt="" id="Best 10 UK Spots logo" title="Best 10 UK Spots logo" />
            </a>
          </div>
          <div id="menu-icon-container" class="flex items-center justify-center" @click="${this.toggleMenu}">
            <div>
              <img src="https://cdn.best10ukspots.com/public/assets/navbar/menu-icon.webp" class="size-7 object-contain" id="menu-icon" alt="Menu icon" />
            </div>
          </div>
        </nav>
